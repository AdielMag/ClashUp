...
- **Perms split:** controller SA `clashup-fleet-controller` holds `compute.instanceAdmin.v1` + `cloudscheduler.admin` + `monitoring.viewer`. Dashboard SA gets only `run.invoker` on the service — it triggers wake but can't touch compute. Dashboard mints an OIDC token (`GoogleCredential.GetOidcTokenAsync`, audience = service URL) for `/api/wake`.
- **Terraform:** `ops/terraform/fleet-controller.tf` (Cloud Run + SA + IAM + scheduler + invoker bindings + `run`/`cloudscheduler` API enablement). Output `fleet_controller_url` → set as dashboard `Gcp:FleetControllerUrl`.
