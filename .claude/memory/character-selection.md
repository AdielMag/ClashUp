...
## Flow
`LobbyEntryPoint.StartAsync`: after Play is clicked (and the lobby UI destroyed), show `CharacterSelectUI`, await its `OnConfirmed(CharacterId)`, store the choice, then `_flow.EnterMatchmaking()`. The reconnect path (`CheckActiveMatchAsync` → `EnterMatchFromLobby`) **skips** selection — the server already remembers the player's `PlayerSummary`.

## Pieces
- **`CharacterSelectUI`** (`Core/Lobby/Scripts/UI/CharacterSelectUI.cs`) — programmatic full-screen overlay (mirrors the `LobbyUI.Create()` pattern: own Canvas + CanvasScaler 1920×1080 + GraphicRaycaster). One card per character with DisplayName + HP + MoveSpeed, highlight on select, CONFIRM button. `static Create(CharactersConfig, CharacterId current)` + `event Action<CharacterId> OnConfirmed` + `Destroy()`. Roster source = compiled-in `CharactersConfig.Default.Characters` (future: a Services RPC for the live DB roster).
