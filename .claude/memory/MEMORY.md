...
- **Per-player point counter**: `PlayerViewSystem` shows a gold points label above each player's nameplate (objective modes only — gated by injected `MatchModeHolder.IsElimination`). `CreatePointsLabel` builds a `TextMeshProUGUI` under the player's WorldUI canvas at anchoredPosition (0,+40) (name is centered, HealthBar at −40), reusing the nameplate's font. Points come from `PlayerStateDto.Points` (already in snapshots), captured for ALL players in `OnSnapshotDecoded` and written each tick via `UpdatePoints`. (Separate from the local-player screen-space `EliminationHudController` "POINTS N".)
...
- [elimination-mode.md](elimination-mode.md) — 2nd game mode: no-respawn + points economy + breakable boxes/orbs + last-team-standing; lobby mode-select buttons, orb collision-layer/id-recycling gotchas, point→HP/damage scaling, arena_pickup
...
- [bot-system.md](bot-system.md) — Server-side AI bots: per-mode bot-fill matchmaking (MatchConfig Keys 5-7), BotDirector wander/chase/attack/flee/seek-box FSM (utility player-vs-box priority), TryGetBotView perception, auto-attacks hit boxes (FindNearestTargetYaw), bot-only cleanup, orange bot nameplates
- [map-visual-builder.md](map-visual-builder.md) — Client procedural arena visuals (grid floor + walls + spawn pads) from baked MapData when a map has no authored prefab; powers the elimination map (arena_pickup), which gained obstacles
