...
- **`AbilityClientInfo.CastShape`** (Key 4, `TelegraphConfig?`): the damage footprint shown by the triggered cast flash. Auto-derived server-side in `ServerAbilityStore.BuildClientConfig` → `DeriveCastShape(root hitbox)`: Capsule hitbox→`{Capsule, Length, Width=2*Radius}`, Cone→`{ForwardCone, Length, Angle}`, Circle→`{TargetCircle, Radius}`. Single source of truth = the hitbox.
...
- **Telegraph shapes**: `CircleAroundCaster`, `TargetCircle`, `ForwardLine`, `ForwardCone`, `Capsule` — direction always follows `AimYaw`. `TelegraphConfig` has `Width` (Key 5) for Capsule/ForwardLine.
