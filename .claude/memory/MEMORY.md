...
- **Editor tool**: `Tools → Ability Editor` (UIToolkit GraphView, `ClashUp.AbilityEditor.asmdef`). Save to the **server** `Abilities/Data/ability_*.json` path only — there is NO client ability-JSON folder; the client gets ability config over the wire (`AbilitiesConfig`). See [[feedback-ability-editor-sync]].
