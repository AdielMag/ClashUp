...
- **Billboard name labels**: `Player.prefab` has a `NameLabel` child (world-space Canvas, scale 0.03) with `BillboardLabel` MonoBehaviour + `TextMeshProUGUI` grandchild. `BillboardLabel` faces `CameraService.Instance.ActiveCamera` in `LateUpdate` — uses the match camera during gameplay, `Camera.main` fallback otherwise.
- **World-space health bar**: `NameLabel` also has a `HealthBar` child (200×16, anchored at y=-40) with a dark background `Image` and a `Fill` child (`Image`, type=Filled, fillMethod=Horizontal, green). `WorldSpaceHealthBar` MonoBehaviour on HealthBar holds a `_fill` Image reference; `SetHealth(current, max)` sets `fillAmount`. Inherits Canvas + BillboardLabel from NameLabel — no extra Canvas needed. Script at `Core/Gameplay/Scripts/UI/WorldSpaceHealthBar.cs`.
- **Per-player component caching in PlayerViewSystem**: Cache MonoBehaviours found on spawn via `GetComponentInChildren<T>()` in a `Dictionary<string, T>`. Remove on `UnregisterPlayer`. Update each frame in `Tick()`. Pattern used for `WorldSpaceHealthBar`.
