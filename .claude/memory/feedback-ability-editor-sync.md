...
**How to apply:** Editor lives at `client/ClashUp.Unity/Assets/Core/Gameplay/Scripts/Editor/AbilityEditor/`. For each new field touch all three: (1) the node UI class (`Nodes/ProjectileNode.cs`, `HitboxNode.cs`, or `RootNode.cs` for telegraph/root-level fields) — add the `FloatField`/`EnumField`/etc. and append to `extensionContainer`; (2) `AbilityGraphSerializer.SerializeGraph`/`BuildHitbox`/`BuildProjectile` — write the field; (3) `AbilityGraphSerializer.DeserializeToGraph`/`PlaceNode` — read it back.

**Confirmed instances of the drop bug:** the editor historically wrote ONLY Id/DisplayName/Cooldown/Button/Telegraph/Root/VisualGuid — so `TriggerMode`, `CastMode`, and `AutoRange` were silently zeroed on every save (e.g. re-saving an Auto attack made it Manual). Fixed 2026-06 (RootNode got Trigger/Cast/AutoRange fields + serializer round-trip). The editor STILL omits `TelegraphConfig.Width` (Capsule-only; pre-existing gap). See [[ability-authoring]], [[target-point-cast]].
