...
<!-- Folder layout: topic files are grouped into subfolders (architecture/, netcode/, abilities/,
...
## Key Paths
- **Server tests**: `src/Server/ClashUp.GameServer.Tests` — consolidated xUnit suite. See [server-test-suite.md](reference/server-test-suite.md).
...
- **UI Toolkit migration**: [ui-toolkit-migration.md](ui/ui-toolkit-migration.md) — runtime UI moved UGUI→UI Toolkit; UXML/USS/fonts/PanelSettings in `Assets/Resources/UI/`

## Client Folder Structure (Unity Assets)
Scripts live in typed subfolders (Interfaces/, Services/, Clients/, Models/, Config/, Scopes/, EntryPoints/, Presenters/, UI/, Receivers/). See [folder-conventions.md](architecture/folder-conventions.md). Assembly definitions + package versions + Match Camera architecture: [project-structure.md](architecture/project-structure.md).
...
## Architecture Rules
- **Dumb client**: NEVER put game logic, state decisions, or authoritative behavior on the client. All game state transitions come from the server; client only renders. See [boot-flow.md](boot/boot-flow.md), [feedback-client-authority.md](feedback/feedback-client-authority.md).
- **Server-authoritative**: server is the single source of truth for all game state.

## Physics, Netcode & Simulation
- **AetherNet physics** (vendored, GC-free deterministic 2D over Aether.Physics2D): [aethernet-architecture.md](netcode/aethernet-architecture.md)
- **Client prediction / server reconciliation / remote interpolation**: [netcode-architecture.md](netcode/netcode-architecture.md)

## Character / Stat / Health / Ability Systems
- **Characters, stats, health, RNG, respawn**: [stat-health-system.md](characters/stat-health-system.md)
- **Character selection UI**: [character-selection.md](characters/character-selection.md)
- **Ability runtime architecture** (nodes, executor, wire protocol, shapes, tuning, VFX): [ability-system-core.md](abilities/ability-system-core.md)
- **Ability JSON authoring/editor tool**: [ability-authoring.md](abilities/ability-authoring.md), [ability-editor-ui.md](abilities/ability-editor-ui.md), [feedback-ability-editor-sync.md](feedback/feedback-ability-editor-sync.md)
- **Projectiles**: [projectile-system.md](abilities/projectile-system.md) | **TargetPoint cast mode**: [target-point-cast.md](abilities/target-point-cast.md)

## Map System
Shared `MapData` POCOs, server/client loaders, baker editor tool, procedural visual builder. See [map-system.md](maps/map-system.md), [map-visual-builder.md](maps/map-visual-builder.md).
...
Joysticks (shared circular touch-zone routing, floating re-anchor, cancel bar, cooldown badges): [joystick-ui.md](ui/joystick-ui.md). General UI Toolkit migration notes/gotchas: [ui-toolkit-migration.md](ui/ui-toolkit-migration.md).

## Android / IL2CPP Build
MagicOnion source-gen requirement, shader stripping, emulator ports. See [android-build.md](ops/android-build.md).

## See Also
- [project-structure.md](architecture/project-structure.md) — Assembly defs, package versions, Match Camera, VContainer scopes, Docker Compose, networking layer, additive scenes
- [scene-ownership.md](architecture/scene-ownership.md) — Domain ownership rules (assets live where their lifespan is)
- [folder-conventions.md](architecture/folder-conventions.md) — Script subfolder rules (Interfaces/, Services/, etc.)
- [patterns.md](architecture/patterns.md) — Code patterns (IDebugLogger, canvas scaler, DI patterns, default-interface-methods for multi-impl interfaces, etc.)
...
- [grass-stealth.md](gameplay/grass-stealth.md) — Grass stealth zones: per-viewer snapshot filtering, MagicOnion `group.Single()`, hidden-DTO wire protocol
- [lobby-ui.md](ui/lobby-ui.md) / [lobby-ui-reskin.md](ui/lobby-ui-reskin.md) — Lobby pager UI + Clash-style reskin
- [gcp-ops-gotchas.md](ops/gcp-ops-gotchas.md) — Deploy/verify gotchas (GITHUB_TOKEN, MIG update policies, Monitoring REST API)
- [deployment-architecture.md](ops/deployment-architecture.md) — Version-aware gateway, on-demand backend spawning, CCU model
- [dashboard-tool.md](ops/dashboard-tool.md) — Local fleet dashboard (`src/Tools/ClashUp.Dashboard`, :8080, NOT deployed)
</content>
